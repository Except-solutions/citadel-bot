...
@future_safe
async def _make_butt_request(butt_url: str) -> list[dict[str, str]]:
    async with httpx.AsyncClient(timeout=5) as client:
        response = await client.get(butt_url)
        response.raise_for_status()
        return response.json()


async def _get_random_butt() -> IOResultE[list[dict[str, str]]]:
    return await flow(
...
async def get_random_butt() -> str:
    ...
    return unsafe_perform_io(
        io_butts.lash(lambda _: IOSuccess([{'preview': DEFAULT_PIC}]))
        .map(
            lambda result: flow(
                result,
                first,
